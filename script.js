...
const collectionSearchBtn = document.getElementById('collection-search-btn');
...
const totalCollectionCountElement = document.getElementById('total-collection-count');
...
function updateTypeFilters() {
    ...
    collectionTypeFilter.innerHTML = '<option value="all">Todos los tipos</option>';
...
function isCardOwned(card, rarity) {
    return inventory.some(invCard => 
        invCard.name === card.name && 
        invCard.rarity === rarity && 
        invCard.type === card.type
    );
}
    ...
    inventory.forEach((card, index) => {
        ...
        if (!groups[key]) {
            groups[key] = {
                ...
                count: 1,
                instances: [card],
                displayKey: `${card.name}-${card.rarity}`
            };
        } else {
            groups[key].count++;
...
function updateInventory() {
    ...
    Object.values(cardGroups).forEach(displayGroup => {
        if (selectedType !== 'all' && displayGroup.card.type !== selectedType) {
            return;
        }
        
        if (searchTerm && !displayGroup.card.name.toLowerCase().includes(searchTerm)) {
            return;
        }
    ...
    if (!hasResults && (selectedType !== 'all' || searchTerm)) {
        cardsContainer.innerHTML = '<p>No se encontraron cartas que coincidan con la búsqueda.</p>';
    }
...
function updateCollection() {
    ...
    const selectedType = collectionTypeFilter.value;
    ...
    Object.keys(cardsByType).sort().forEach(type => {
        const typeCards = cardsByType[type];
        const filteredCards = typeCards.filter(card => {
        ...
        const collectedInType = filteredCards.filter(card => card.owned).length;
        totalCards += filteredCards.length;
        collectedCards += collectedInType;
        
        typeSection.innerHTML = `
            <div class="type-header">
                <h3>${type} (${collectedInType}/${filteredCards.length})</h3>
            </div>
            <div class="type-cards">
                ${filteredCards.map(card => `
                    <div class="card ${card.owned ? '' : 'missing'}">
                        ...
                        ${!card.owned ? '<div class="card-missing">No obtenida</div>' : ''}
                    </div>
                `).join('')}
            </div>
        `;
    ...
    collectedCountElement.textContent = collectedCards;
    totalCollectionCountElement.textContent = totalCards;
    collectionProgressElement.textContent = `${progress}%`;
    
    if (Object.keys(cardsByType).length === 0) {
        collectionContainer.innerHTML = '<p>No hay cartas en la colección.</p>';
    }
}
...
function editCustomCard(rarity, index) {
    const card = JSON.parse(JSON.stringify(customCards[rarity][index]));
...
confirmEditBtn.addEventListener('click', () => {
    if (currentEditCard && currentEditRarity !== null && currentEditIndex !== null) {
        ...
        if (newRarity !== currentEditRarity) {
            ...
            customCards[newRarity].push(originalCard);
        }
...
collectionTypeFilter.addEventListener('change', updateCollection);

inventorySearchBtn.addEventListener('click', updateInventory);
inventorySearch.addEventListener('input', updateInventory);
collectionSearchBtn.addEventListener('click', updateCollection);
